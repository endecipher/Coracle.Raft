...
namespace Core.Raft.Canoe.Engine.Actions
{
    ...
    /// </summary>
    internal sealed class OnSendAppendEntriesRPC : EventAction<OnSendAppendEntriesRPCLogsContext, IAppendEntriesRPCResponse>
    {
        #region Constants
        ...
        protected override async Task<IAppendEntriesRPCResponse> Action(CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                var previousLogEntry = await Input.PersistentState.LogEntries.TryGetValueAtIndex(nextIndex - 1);
