...
namespace Coracle.Samples.PersistentData
{
    ...
    {
        #region Constants 
        public const string Entity = nameof(TestStateProperties);
        ...
        public TestStateProperties(IActivityLogger activityLogger, ISnapshotManager snapshotManager, IClusterConfiguration clusterConfiguration, IEngineConfiguration engineConfiguration)
        {
            CurrentTerm = 0;
        ...
        public Task<(bool HasSnapshot, ISnapshotHeader SnapshotDetail)> HasCommittedSnapshot(long inclusiveIndex)
        {
            lock (obj)
            {
                ...
                var isIndexZero = inclusiveIndex == default(long);
        ...
        {
            if (await IsInputSnapshotCommittedAndAvailableInLog(snapshot))
            {
        ...
        public async Task<bool> IsEligibleForCompaction(long commitIndex, long lastApplied, TimeSpan waitPeriodMs)
        {
            ...
            lock (obj)
            {
                ...
                return compactableNodeCount >= EngineConfiguration.SnapshotThresholdSize + EngineConfiguration.SnapshotBufferSizeFromLastEntry;
            }
        }

        public async Task<ISnapshotHeader> Compact(long commitIndex, long lastApplied)
        {
            ...
            lock (obj)
            {
                ...
                while (currentElement != null && currentElement.Entry.CurrentIndex <= lastApplied)
                {
                    ...
                    if (entriesToMerge.Count == EngineConfiguration.SnapshotThresholdSize)
                    {
            ...
            if (reachedThreshold)
            {
                var uncomittedHeader = await SnapshotManager.CreateFile(entriesToMerge, ClusterConfiguration.CurrentConfiguration);
        ...
        public async Task<bool> IsInputSnapshotCommittedAndAvailableInLog(ISnapshotHeader snapshotDetail)
        {
        ...
        public async Task<(ISnapshotChunkData Data, bool IsLastOffset)> TryGetSnapshotChunk(ISnapshotHeader snapshotDetail, int offsetToFetch)
        {
            var file = await SnapshotManager.GetFile(snapshotDetail);
        ...
        public async Task<ISnapshotHeader> FillSnapshotData(string snapshotId, long lastIncludedIndex, long lastIncludedTerm, int fillAtOffset, ISnapshotChunkData receivedData)
        {
