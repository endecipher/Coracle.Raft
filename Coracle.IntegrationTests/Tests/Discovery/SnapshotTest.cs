...
namespace Coracle.IntegrationTests.Framework
{
    ...
    public class SnapshotTest : BaseTest, IClassFixture<TestContext>
    {
        public SnapshotTest(TestContext context) : base(context)
        {
        }
        ...
        public async Task IsLeaderHandlingConfigurationChangeAndSendingSnapshot()
        {
            ...
            var commitIndexUpdated = notifiableQueue
                .AttachNotifier(x =>
                    x.Is(AbstractState.Entity, AbstractState.ApplyingLogEntry)).RemoveOnceMatched();
            ...
            var snapshotDetail = await Context.GetService<IPersistentProperties>().GetCommittedSnapshot();
